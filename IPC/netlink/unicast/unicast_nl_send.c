...
int main() {
  ...
  // allocate buffer for netlink message which is message header + message
  // payload
  ...
  src_addr.nl_pid = 32;            // application unique id
  ...
  nlh->nlmsg_pid = 1010;                       // src application unique id
  nlh->nlmsg_flags = 0;
  ...
  dest_addr.nl_pid = 64;            // destination process id
